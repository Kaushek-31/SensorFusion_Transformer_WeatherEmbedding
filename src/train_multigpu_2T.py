    ...
    for epoch in range(lepochs,nepochs):
        ...
        for batchi, (imgs, rots, trans, intrins, post_rots, post_trans, binimgs, points) in enumerate(trainloader):
            ...
            if counter % val_step == 0:
                ...
                if gpuid == 0:
                    val_info = get_val_info(model, valloader, loss_fn, device, cfg=cfg.cfg_pp, is_training=True, use_tqdm=True)
                    
                    print('VAL', val_info['iou']) 
                    print('LOSS', val_info['loss'])
...
def main():
    ...
    # Set CUDA_VISIBLE_DEVICES before any TensorFlow calls
    #os.environ['CUDA_VISIBLE_DEVICES'] = ','.join([str(i+1) for i in range(args.gpus)])
