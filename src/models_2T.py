...
if gpus:
    try:
        # Set only GPU 1 as visible (index 1)
        tf.config.set_visible_devices(gpus[0], 'GPU')
        tf.config.experimental.set_memory_growth(gpus[0], True)  # Optional: prevent full memory allocation
...
# Load models
night_model_path='/home/kaushek/TFGrid/models/NightForest.sav'
precipitation_model_path='/home/kaushek/TFGrid/models/PrecipitationCNN.h5'
fog_model_path='/home/kaushek/TFGrid/models/FogCNN.h5'
...
class LearnableAttention(nn.Module):
    ...
    def __init__(self, n_embd, n_head, attn_pdrop, resid_pdrop, mask_init='zeros', mask_scale=1.0):
        super().__init__()
        assert n_embd % n_head == 0
        self.n_head = n_head
        ...
        # Learnable attention mask (initialized with the specified method)
        if mask_init == 'zeros':
            self.mask = nn.Parameter(torch.zeros(1, n_head, 1, 1), requires_grad=True)
        elif mask_init == 'random':
            self.mask = nn.Parameter(torch.randn(1, n_head, 1, 1) * mask_scale, requires_grad=True)
        else:
            raise ValueError("Invalid mask initialization method")
...
class GPT(nn.Module):
    ...
    def forward(self, image_tensor, lidar_tensor, weather_condition, fog_condition, device):
        """
        Args:
        ...
        #weather_fin = torch.tensor(weather_fin).to(device)
        
        #weather_embedding = self.weather_emb(torch.tensor([weather_fin], dtype=torch.long).to(device)).unsqueeze(1)
        weather_condition_tensor = torch.tensor([weather_fin], dtype=torch.long, device=image_tensor.device)
...
class Transfuser(nn.Module):
    ...
    def forward(self, image_list, lidar_list, weather_class, fog_class, device):
        '''
        ...
        image_features_layer1, lidar_features_layer1 = self.transformer1(image_embd_layer1, lidar_embd_layer1, weather_class, fog_class, device)
        ...
        image_features_layer2, lidar_features_layer2 = self.transformer2(image_embd_layer2, lidar_embd_layer2, weather_class, fog_class, device)
...
class LPT(nn.Module):
    def __init__(self, grid_conf, data_aug_conf, outC,pp_config,tf_config):
        ...
        self.night_model = pickle.load(open(night_model_path, 'rb'))
        self.precipitation_model = tf.keras.models.load_model(precipitation_model_path)
        self.fog_model = tf.keras.models.load_model(fog_model_path)

        # toggle using QuickCumsum vs. autograd
        self.use_quickcumsum = True
    
    def run_inference_single_image(self, image, device):
        """
        ...
        # Prediction
        night_prediction = self.night_model.predict([features])
        precipitation_prediction = self.precipitation_model.predict(np.expand_dims(image_resized, axis=0), verbose=0)
        fog_prediction = self.fog_model.predict(np.expand_dims(image_resized, axis=0), verbose=0)
    ...
    def forward(self, x, rots, trans, intrins, post_rots, post_trans, voxels, coors, num_points):
        ...
        img_np = img.permute(1, 2, 0).cpu().numpy()
        weather_class, fog_class = self.run_inference_single_image(img_np, x.device)
        ...
        x = self.transfuser(x,pointpillars_features, weather_class, fog_class, x.device)#transfuser        
